...
figure(2);
set(2,'color', 'w','windowstyle','docked');
...
% set min windowing size
minWindowSize = 50;

% set noise floor for window
dbWindow = 30;
...
while or((max(max(abs(p(:,:)))) > (p0 * 10^(40/10))),(n < 48000))
    ...
    [idx] = SPARSEfun1D(p, dbWindow);
    [size(idx(idx > 0),2) (size(idx(idx > 0),2)/size(p,2)*100)]
    [p, ux] = SFDTD1Dfun(p, pCx, ux, uCx, Rx, ZL, ZR, idx, minWindowSize);
